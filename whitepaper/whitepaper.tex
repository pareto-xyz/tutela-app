...
If the address making a deposit transaction to a Tornado Cash pool matches the address making a withdrawal transaction (from eithe the same pool or a different pool), the two transactions can be linked, and the corresponding deposit is compromised, as the user identity is revealed. These may be TORN yield farmers who deposit and withdraw to the same address and are only profit motivated, or, clumsy Tornado cash users.
